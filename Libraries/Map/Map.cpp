...
uint8_t Map::getUnvisitedTiles() {
  ...
  for (int row = 0 ; row < numRows ; row++) {
    for (int column = 0 ; column < numColumns ; column++) {
      if (!map[row][column].isVisited() &&
          map[row][column].isAccessible() &&
          !map[row][column].isBlack()) {
        unvisitedTiles++;
      }
    }
  }
...
void Map::setTile(Tile tile, char position) {
  if (position == '0') {
      map[currRow][currColumn] = tile;
  }
  else if (position == 'N') {
      map[currRow - 1][currColumn] = tile;
  }
  else if (position == 'E') {
      map[currRow][currColumn + 1] = tile;
  }
  else if (position == 'S') {
      map[currRow + 1][currColumn] = tile;
  }
